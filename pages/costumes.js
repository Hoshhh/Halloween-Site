...
export default function Costumes({ costumes }) {
            ...
            <div className="container text-white grid place-content-center 2xl:grid-cols-3 2xl:grid-rows-2 lg:grid-cols-2 lg:grid-rows-2 h-full pt-4">
                {
                    costumes.map((costume) => {
                        return(
                            <ImageCard key={costume.id} title={costume.title} img={costume.img} url={costume.link} />
                        )
                    })
                }
            </div>
...
export async function getServerSideProps() {
    const response = await fetch('http://localhost:3000/api/costumes')
